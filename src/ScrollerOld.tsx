...
export const Scroller = forwardRef<ScrollerRef, Props>(
  (
    ...
    const rafPidRef = useRef<NodeJS.Timeout>();
    ...
    const scrollerRef: ScrollerRef = {
      ...
      setOffset(offset) {
        const cellsElmnt = cellsElmntRef.current || document.querySelector(`.${rootElmntClassName} > .nonSticky > div`);
        const stickyRowsElmnt =
          stickyRowsElmntRef.current || document.querySelector(`.${rootElmntClassName} > .stickyRows > div`);
        const stickyColsElmnt =
          stickyColsElmntRef.current || document.querySelector(`.${rootElmntClassName} > .stickyCols > div`);
    ...
    useEffect(() => {
      ...
      for (let i = fromRow; i < toRow; i += 1) {
        ...
        if (isNatural) {
          const cellElmnts = Array.prototype.slice.call(document.querySelectorAll(`.r${i}`)) as HTMLElement[];
          const maxHeight = cellElmnts.reduce(
            (max, cellElmnt) => Math.max(max, cellElmnt.getBoundingClientRect().height),
            0,
          );
          if (maxHeight !== heightOverrides[i]) {
    ...
    useEffect(() => {
      ...
      for (let i = fromCol; i < toCol; i += 1) {
        ...
        if (isNatural) {
          const cellElmnts = Array.prototype.slice.call(document.querySelectorAll(`.c${i}`)) as HTMLElement[];
          const maxWidth = cellElmnts.reduce(
            (max, cellElmnt) => Math.max(max, cellElmnt.getBoundingClientRect().width),
            0,
          );
          if (maxWidth !== widthOverrides[i]) {
      ...
      if (rootElmnt) {
        const resizeObserver = new ResizeObserver(() => {
          ...
          resizePidRef.current = setTimeout(() => {
            ...
            const effWidth = width - padding.left - padding.right;
            const effHeight = height - padding.top - padding.bottom;
            if (effWidth !== windowSizeRef.current.width || effHeight !== windowSizeRef.current.height) {
              windowSizeRef.current = { width: effWidth, height: effHeight };
              reflow();
              setRenderFlag(rf => !rf);
            }
    ...
    const setOffset = useCallback(
      (
        ...
        if (
          ...
          const visibleTo = {
            row: sizeToFit.current.height ? numRows : rowOffsets.findIndex(rowOffset => rowOffset >= offset.y + height),
            col: sizeToFit.current.width ? numCols : colOffsets.findIndex(colOffset => colOffset >= offset.x + width),
          };
          if (visibleTo.row === -1) {
          ...
          if (rafPidRef.current) {
            clearTimeout(rafPidRef.current);
          }
          rafPidRef.current = setTimeout(() => {
            // rafPidRef.current = requestAnimationFrame(() => {
            rafPidRef.current = undefined;
            ...
            hScrollElmntsRef.current.forEach(([c, hScrollElmnt]) => {
              hScrollElmnt.style.transform = `translateX(${stuckCols.includes(c) ? 0 : translateX}px)`;
            });

            vScrollElmntsRef.current.forEach(([r, vScrollElmnt]) => {
              vScrollElmnt.style.transform = `translateY(${stuckRows.includes(r) ? 0 : translateY}px)`;
            });
    ...
    useEffect(() => {
      ...
      const source = (eventSource || eventSourceRef?.current || rootElmntRef.current) as HTMLElement | undefined;
      ...
      const scroll = (deltaX: number, deltaY: number) => {
        ...
        if (totalSize.width <= windowSizeRef.current.width && totalSize.height <= windowSizeRef.current.height) {
          return;
        }
        ...
        if (rootElmnt) {
          ...
          const effectiveDeltaY =
            sizeToFit.current.height || !((deltaY > 0 && isBottomVisible) || (deltaY < 0 && isTopVisible)) ? 0 : deltaY;
    ...
    }, [totalSize.width, totalSize.height, maxOffset.x, maxOffset.y, scrollSpeed, freezeScroll, passive]);
    ...
    const visibleCells = useMemo(() => {
      const cells: boolean[][] = new Array(numRows).fill(null).map(() => new Array(numCols).fill(true));
      ...
      return cells;
    }, [rows, numCols, numRows, fromRow, toRow, fromCol, toCol, stuckRows.join(), stuckCols.join()]);
    ...
    const makeRenderedCell = (r: number, c: number) => {
      const renderedCell = visibleCells[r][c] && children(rows[r][c], { data: rows[r][c], row: r, col: c });
      if (renderedCell) {
        const span = cellSpan({ data: rows[r][c], row: r, col: c });

        const cellWidthOverridePx =
          span.cols === 1 ? undefined : colWidths.slice(c, c + span.cols).reduce((cw, w) => cw + w, 0);

        const cellHeightOverridePx =
          span.rows === 1 ? undefined : rowHeights.slice(r, r + span.rows).reduce((ch, h) => ch + h, 0);
    ...
    for (let r = fromRow; r < toRow; r += 1) {
      if (!stuckRows.includes(r) && !suppressHScrollRows.includes(r)) {
        ...
        if (isNatural) {
          cellStyles.push(`.${rootElmntClassName} > .window > div > .r${r} { top: ${px(rowTop)}; }`);
        } else {
          cellStyles.push(
            `.${rootElmntClassName} > .window > div > .r${r} { top: ${px(rowTop)}; height: ${px(height)}; }`,
          );
        }
    ...
    for (let c = fromCol; c < toCol; c += 1) {
      if (!stuckCols.includes(c)) {
        ...
        if (isNatural) {
          cellStyles.push(`.${rootElmntClassName} > .window > div > .c${c} { left: ${px(colLeft)}; }`);
        } else {
          cellStyles.push(
            `.${rootElmntClassName} > .window > div > .c${c} { left: ${px(colLeft)}; width: ${px(width)}; }`,
          );
        }
      }
    }
    ...
    stuckRows.forEach(r => {
      ...
      if (!suppressHScrollRows.includes(r)) {
        cellStyles.push(
          `.${rootElmntClassName} > .stickyRows > div > .r${r} { top: ${px(stuckRowTop)}; height: ${px(rowHeight)}; }`,
        );
        for (let c = fromCol; c < toCol; c += 1) {
          if (!stuckCols.includes(c) && !suppressVScrollCols.includes(c)) {
            const cell = makeRenderedCell(r, c);
    ...
    stuckCols.forEach(c => {
      ...
      if (!suppressVScrollCols.includes(c)) {
        cellStyles.push(
          `.${rootElmntClassName} > .stickyCols > div > .c${c} { left: ${px(stuckColLeft)}; width: ${px(colWidth)}; }`,
        );
        for (let r = fromRow; r < toRow; r += 1) {
          if (!stuckRows.includes(r) && !suppressHScrollRows.includes(r)) {
            const cell = makeRenderedCell(r, c);
    ...
    stuckRows.forEach(r => {
      const rowHeight = rowHeights[r];
      cellStyles.push(
        `.${rootElmntClassName} > .stickyCells > .r${r} { top: ${px(stuckRowTop)}; height: ${px(rowHeight)}; }`,
      );
      stuckRowTop += rowHeight;
    });
    ...
    stuckCols.forEach(c => {
      const colWidth = colWidths[c];
      cellStyles.push(
        `.${rootElmntClassName} > .stickyCells > .c${c} { left: ${px(stuckColLeft)}; width: ${px(colWidth)}; }`,
      );
      stuckColLeft += colWidth;
    });
    ...
    suppressHScrollRows.forEach(r => {
      ...
      while (colOffsets[c] + colWidths[c] < windowSizeRef.current.width) {
        ...
        if (cell) {
          ...
          cellStyles.push(
            `.${rootElmntClassName} > .r${r}.c${c} { position: absolute; left: ${px(colOffset)}; top: ${px(
              padding.top + top,
            )}; width: ${px(colWidths[c])}; max-width: 100%; height: ${px(
              rowHeights[r],
            )}; box-sizing: border-box; z-index: ${stuckRows.includes(r) ? 1 : 0}; }`,
          );
        }
        c += span.cols;
      }
    ...
    suppressVScrollCols.forEach(c => {
      ...
      while (rowOffsets[r] + rowHeights[r] < windowSizeRef.current.height) {
        ...
        if (cell) {
          ...
          cellStyles.push(
            `.${rootElmntClassName} > .r${r}.c${c} { position: absolute; top: ${px(rowOffset)}; left: ${px(
              padding.left + left,
            )}; width: ${px(colWidths[c])}; max-width: 100%; height: ${px(
              rowHeights[r],
            )}; box-sizing: border-box; z-index: ${stuckCols.includes(c) ? 1 : 0}; }`,
          );
        }
        r += span.cols;
      }
    ...
    const naturalHeight =
      sizeToFit.current.height && renderWindowRef.current.toRow > 0 ? px(totalSize.height) : undefined;
    const naturalWidth = sizeToFit.current.width && renderWindowRef.current.toCol > 0 ? px(totalSize.width) : undefined;
    ...
    return (
      <>
        ...
        >
          ...
          <div
            className={[stickyClassName, 'window', 'stickyCols', !hasStuckCols && 'empty'].filter(Boolean).join(' ')}
          >
          ...
          <div
            className={[stickyClassName, 'window', 'stickyRows', !hasStuckRows && 'empty'].filter(Boolean).join(' ')}
          >
          ...
          <div
            className={['cells', 'stickyCells', !(hasStuckRows && hasStuckCols) && 'empty', stickyClassName]
              .filter(Boolean)
              .join(' ')}
          >
            {stuckCells}
